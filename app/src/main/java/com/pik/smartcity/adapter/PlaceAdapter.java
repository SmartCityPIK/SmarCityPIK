package com.pik.smartcity.adapter;
...
public class PlaceAdapter extends BaseAdapter implements IWhereMyLocationConstants {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = mInflater.inflate(com.pik.smartcity.R.layout.item_location, null);
            convertView.setTag(mHolder);
        } else {
            mHolder = (ViewHolder) convertView.getTag();
        }
        mHolder.mTvName = (TextView) convertView.findViewById(com.pik.smartcity.R.id.tv_name_location);
        mHolder.mTvVicinity = (TextView) convertView.findViewById(com.pik.smartcity.R.id.tv_sumary);
        mHolder.mTvDistance = (TextView) convertView.findViewById(com.pik.smartcity.R.id.tv_distance);
        mHolder.mImgPlace = (ImageView) convertView.findViewById(com.pik.smartcity.R.id.img_location);
        mHolder.mRatingBar = (RatingBar) convertView.findViewById(com.pik.smartcity.R.id.ratingBar1);
        ...
        if (!StringUtils.isStringEmpty(mPlaceObject.getVicinity())) {
            ...
            mHolder.mTvVicinity.setText(com.pik.smartcity.R.string.title_unknown_address);
        }
        ...
        if (StringUtils.isStringEmpty(distance)) {
            ...
            if (metric.equals(UNIT_KILOMETTER)) {
                distance = String.format(mContext.getString(com.pik.smartcity.R.string.info_distance_km_format), String.valueOf(mPlaceObject.getDistance()));
            } else if (metric.equals(UNIT_MILE)) {
                float convertDistance = Math.round(mPlaceObject.getDistance() / ONE_MILE);
                distance = String.format(mContext.getString(com.pik.smartcity.R.string.info_distance_miles_format), String.valueOf(convertDistance));
            }
        }
