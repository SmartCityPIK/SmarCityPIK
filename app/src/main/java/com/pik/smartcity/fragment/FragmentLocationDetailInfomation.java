...
import com.pik.smartcity.DetailLocationAcitivity;
import com.pik.smartcity.ShowUrlActivity;
import com.pik.smartcity.constanst.IWhereMyLocationConstants;
import com.pik.smartcity.dataMng.TotalDataManager;
import com.pik.smartcity.object.PlaceDetailObject;
import com.pik.smartcity.object.PlaceObject;
import com.pik.smartcity.object.PlacePhotoObject;
import com.pik.smartcity.object.UserReviewObject;
import com.pik.smartcity.settings.SettingManager;
...
public class FragmentLocationDetailInfomation extends Fragment implements IWhereMyLocationConstants {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        mRootView = inflater.inflate(com.pik.smartcity.R.layout.fragment_location_detail_infomation, container, false);
        return mRootView;
    }
    ...
    private void findView() {
        ...
        this.mCbFavorite = (CheckBox) mRootView.findViewById(com.pik.smartcity.R.id.cb_add_favorite);
        this.mTvNameLocation = (TextView) mRootView.findViewById(com.pik.smartcity.R.id.tv_name_location);
        this.mRatingBar = (RatingBar) mRootView.findViewById(com.pik.smartcity.R.id.ratingBar1);
        this.mTvNumberReviews = (TextView) mRootView.findViewById(com.pik.smartcity.R.id.tv_number_reviews);

        this.mTvTitleImages = (TextView) mRootView.findViewById(com.pik.smartcity.R.id.tv_title_img);
        this.mTvTitlePlaceInfos = (TextView) mRootView.findViewById(com.pik.smartcity.R.id.tv_title_info);
        this.mTvTitleUserReviews = (TextView) mRootView.findViewById(com.pik.smartcity.R.id.tv_title_user_reviews);
        ...
        this.mLayoutImg = (LinearLayout) mRootView.findViewById(com.pik.smartcity.R.id.layout_img);
        this.mLayoutDetailInfo = (LinearLayout) mRootView.findViewById(com.pik.smartcity.R.id.layout_detail_info);
        this.mLayoutDetailReviews = (LinearLayout) mRootView.findViewById(com.pik.smartcity.R.id.layout_detail_reviews);
        ...
        if (mPlaceDetailObject != null) {
            ...
            if (mListReviewObjects != null && mListReviewObjects.size() > 0) {
                ...
                String review = "";
                if (size > 1) {
                    review = String.format(mContext.getString(com.pik.smartcity.R.string.format_reviews), String.valueOf(size));
                } else {
                    review = String.format(mContext.getString(com.pik.smartcity.R.string.format_review), String.valueOf(size));
                }
            ...
            mCbFavorite.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mCbFavorite.isChecked()) {
                        String info = String.format(getString(com.pik.smartcity.R.string.format_add_favorite), mContext.mCurrentPlaceObject.getName());
                        TotalDataManager.getInstance().addFavoritePlace(mContext, mContext.mCurrentPlaceObject);
                        mContext.showToast(info);
                    } else {
                        String info = String.format(getString(com.pik.smartcity.R.string.format_remove_favorite), mContext.mCurrentPlaceObject.getName());
    ...
    private void addListImage(PlaceDetailObject mPlaceDetailObject, PlaceObject mPlaceObject) {
        ...
        if (mListPlacePhotos != null && mListPlacePhotos.size() > 0) {
            for (PlacePhotoObject mPlacePhotoObject : mListPlacePhotos) {
                View mView = LayoutInflater.from(mContext).inflate(com.pik.smartcity.R.layout.item_image, null);
                ImageView mImgView = (ImageView) mView.findViewById(com.pik.smartcity.R.id.item_image);
    ...
    private void setUpInfomation(PlaceDetailObject mPlaceDetailObject, PlaceObject mPlaceObject) {
        ...
        if (!StringUtils.isStringEmpty(address)) {
            addItemInfomation(address, com.pik.smartcity.R.drawable.icon_address, null, false, false);
        }
        String distance = "";
        if (mContext.mRouteObject == null) {
            ...
            if (metric.equals(UNIT_KILOMETTER)) {
                distance = String.format(mContext.getString(com.pik.smartcity.R.string.format_distance), String.valueOf(mPlaceObject.getDistance()) + " km");
            } else if (metric.equals(UNIT_MILE)) {
                float convertDistance = Math.round(mPlaceObject.getDistance() / ONE_MILE);
                distance = String.format(mContext.getString(com.pik.smartcity.R.string.format_distance), String.valueOf(convertDistance) + " mi");
            }
        } else {
            distance = getString(com.pik.smartcity.R.string.title_distance) + ": " + mContext.mRouteObject.getDistance();
        }
        if (!StringUtils.isStringEmpty(distance)) {
            addItemInfomation(distance, com.pik.smartcity.R.drawable.icon_distance, null, false, false);
        }
        final String website = mPlaceDetailObject.getWebsite();
        if (!StringUtils.isStringEmpty(website)) {
            addItemInfomation(website, com.pik.smartcity.R.drawable.icon_website, new IDBCallback() {
        ...
        if (!StringUtils.isStringEmpty(phone)) {
            addItemInfomation(phone, com.pik.smartcity.R.drawable.icon_phone, new IDBCallback() {
                @Override
                public void onAction() {
        ...
        String direction = mContext.getString(com.pik.smartcity.R.string.title_get_directions);
        if (!StringUtils.isStringEmpty(direction)) {
            addItemInfomation(direction, com.pik.smartcity.R.drawable.icon_directions, new IDBCallback() {
    ...
    public void updateDistance() {
        if (isNeedUpdateDistance) {
            if (mContext.mRouteObject != null) {
                if (mTvDistance != null) {
                    mTvDistance.setText(getText(com.pik.smartcity.R.string.title_distance) + ": " + mContext.mRouteObject.getDistance());
    ...
    private void addItemInfomation(String name, int resIconId, final IDBCallback mDBCallback, boolean isEnded, boolean isShowIconMore) {
        RelativeLayout mRelativeLayout = (RelativeLayout) LayoutInflater.from(mContext).inflate(com.pik.smartcity.R.layout.item_information, null);
        TextView mTvName = (TextView) mRelativeLayout.findViewById(com.pik.smartcity.R.id.tv_name);
        ImageView mImgIcon = (ImageView) mRelativeLayout.findViewById(com.pik.smartcity.R.id.img_icon);
        ImageView mImgIconMore = (ImageView) mRelativeLayout.findViewById(com.pik.smartcity.R.id.img_indicator);
        ...
        if (resIconId == com.pik.smartcity.R.drawable.icon_distance) {
            this.mTvDistance = mTvName;
        }
    }

    private void addItemReviews(String nameUser, String content, float rating, long time) {
        RelativeLayout mRelativeLayout = (RelativeLayout) LayoutInflater.from(mContext).inflate(com.pik.smartcity.R.layout.item_review, null);
        TextView mTvNameUser = (TextView) mRelativeLayout.findViewById(com.pik.smartcity.R.id.tv_name_user);
        mTvNameUser.setTypeface(mContext.mTypeFaceRobotoBold);
        mTvNameUser.setText(nameUser);

        TextView mTvContent = (TextView) mRelativeLayout.findViewById(com.pik.smartcity.R.id.tv_content);
        mTvContent.setTypeface(mContext.mTypeFaceRobotoLight);
        mTvContent.setText(Html.fromHtml(content));

        RatingBar mRatingBar = (RatingBar) mRelativeLayout.findViewById(com.pik.smartcity.R.id.user_ratingBar1);
        mRatingBar.setRating(rating);

        TextView mTvTime = (TextView) mRelativeLayout.findViewById(com.pik.smartcity.R.id.tv_date);
